...
function normalizeStockFilter(
  ...
  return {
    ...
    options:
      stockFilter?.options && stockFilter.options.length > 0
        ? [...stockFilter.options]
        : [],
  }
}
...
export function preferencesToFormValues(
  ...
  return {
    ...
    commodities: preferences.commodities.map((commodity) => ({
      name: commodity.name,
      varieties:
        commodity.varieties.length > 0 ? [...commodity.varieties] : [""],
      sizes: commodity.sizes.length > 0 ? [...commodity.sizes] : [""],
    })),
    customFields: Object.entries(preferences.customFields ?? {}).map(
      ([key, value]) => ({
        key,
        value:
