      ...
      .superRefine((rows, ctx) => {
        ...
        if (!hasActiveRow) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message:
              "Enter at least one bag quantity with chamber, floor, and row.",
            path: [],
          })
        }
      }),
...
export type IncomingQuantityLocation = Pick<
  IncomingQuantityRow,
  "chamber" | "floor" | "row"
>

export function hasCompleteIncomingQuantityLocation(
  row: IncomingQuantityLocation
): boolean {
  return (
    row.chamber.trim() !== "" &&
    row.floor.trim() !== "" &&
