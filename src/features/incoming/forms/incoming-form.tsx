...
export function IncomingForm({
  ...
  const formSchema = useMemo(
    () => createIncomingFormSchema(schemaConfig),
    [schemaConfig]
  )
  ...
  const coldStorageId = useStoreAdminStore(
    (s) => s.storeAdmin?.coldStorageId
  )
  ...
  const handleCreateSubmit = useCallback(
    async (values: IncomingFormValues) => {
      try {
        ...
        toast.success(
          `Incoming gate pass #${created.gatePassNo.toLocaleString("en-IN")} created`,
  ...
  const formId =
    mode === "edit" ? "edit-incoming-form" : "create-incoming-form"
  const fieldIdPrefix =
    mode === "edit" ? "edit-incoming" : "create-incoming"
  ...
  const getFarmerCostPerBag = useCallback(
    (farmerIncomingLinkId: string) =>
      farmerStorageLinks.find((link) => link._id === farmerIncomingLinkId)
        ?.costPerBag,
    [farmerStorageLinks]
  )
  ...
  const handleFarmerAdded = useCallback(
    (link: FarmerStorageLink) => {
      ...
      setFarmerSearch(
        `${getLinkDisplayName(link)} — Acct #${link.accountNumber}`
      )
  ...
  const title =
    mode === "edit" ? "Edit Incoming Gate Pass" : "Incoming Gate Pass"
  const description =
    mode === "edit"
  ...
  return (
    <Card className="mx-auto w-full max-w-4xl shadow-sm">
        ...
        <CardTitle className="font-heading text-xl font-semibold tracking-tight sm:text-2xl">
          ...
          {gatePassNoLoading ? (
            ...
            <span className="text-primary tabular-nums">
              #{gatePassNo.toLocaleString("en-IN")}
            </span>
          )}
        </CardTitle>
        {gatePassNoError ? (
      ...
      <form id={formId} noValidate onSubmit={(e) => e.preventDefault()}>
          ...
          <FieldGroup className="@container/field-group gap-6">
            <FieldSet>
              ...
              <FieldGroup className="mt-4 grid grid-cols-1 gap-4 @md/field-group:grid-cols-2">
                ...
                <form.Field
                  name="date"
                  validators={{ onChange: formSchema.shape.date }}
                >
                  {(field) => {
                    const isInvalid = isFieldInvalid(field.state.meta)
                    return (
                ...
                >
                  {(field) => {
                    ...
                    return (
                      ...
                      >
                        <FieldLabel htmlFor={`${fieldIdPrefix}-farmer`}>
                          Farmer
                        </FieldLabel>
            ...
            <FieldSet>
              ...
              <FieldGroup className="mt-4 grid grid-cols-1 gap-4 @md/field-group:grid-cols-2">
                {showCommoditySelect ? (
                  ...
                  >
                    {(field) => {
                      ...
                      return (
                        <Field data-invalid={isInvalid}>
                          <FieldLabel htmlFor={`${fieldIdPrefix}-commodity`}>
                            Commodity
                          </FieldLabel>
                          <SearchableOptionCombobox
                ...
                <form.Field
                  name="variety"
                  validators={{ onChange: formSchema.shape.variety }}
                >
                  {(field) => {
                    const isInvalid = isFieldInvalid(field.state.meta)
                    return (
                      <Field data-invalid={isInvalid}>
                        <FieldLabel htmlFor={`${fieldIdPrefix}-variety`}>
                          Variety
                        </FieldLabel>
                        <SearchableOptionCombobox
        ...
        <CardFooter className="justify-end gap-3 border-t bg-muted/30 px-4 py-6 sm:px-6">
        ...
        children={({ values, canSubmit, isSubmitting }) => {
          ...
          return (
            <IncomingSummarySheet
              ...
              costPerBag={
                parsed.success
                  ? getFarmerCostPerBag(parsed.data.farmerIncomingLinkId)
                  : undefined
              }
