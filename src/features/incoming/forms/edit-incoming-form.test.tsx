...
vi.mock('@/features/auth/store/use-store-admin-store', () => ({
  useStoreAdminStore: (selector: (state: unknown) => unknown) =>
    mockUseStoreAdminStore(selector),
}));
...
function setupFarmerLinks(
  links = [makeFarmerStorageLink()],
  isLoading = false,
) {
...
describe('EditIncomingForm', () => {
  ...
  it('shows a closed gate pass message when status is not OPEN', async () => {
    mockUseIncomingDaybookEntry.mockReturnValue(
      makeIncomingDaybookEntry({ status: 'CLOSED' }),
    );
