...
function locationSignature(quantities: IncomingQuantityRow[]): string {
  return quantities
    .map((row) => `${row.id}|${row.chamber}|${row.floor}|${row.row}`)
    .join(";")
}
...
export function useCompleteLocationOrder(quantities: IncomingQuantityRow[]) {
  ...
  const completeLocationOrder = useMemo(() => {
    ...
    for (const row of quantities) {
      if (
        hasCompleteIncomingQuantityLocation(row) &&
        !next.includes(row.id)
      ) {
        next.push(row.id)
      }
    }
  ...
  const sourceRow = quantities.find(
    (row) => row.id === completeLocationOrder[0]
  )
