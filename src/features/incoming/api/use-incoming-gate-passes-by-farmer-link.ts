...
export function incomingGatePassesByFarmerLinkQueryKey(
  farmerStorageLinkId: string
) {
  return [
    ...INCOMING_GATE_PASSES_BY_FARMER_LINK_QUERY_KEY,
    farmerStorageLinkId,
  ] as const
}
...
async function fetchIncomingGatePassesByFarmerLink(
  ...
  try {
    ...
    throw new Error(
      getApiErrorMessage(error, "Failed to load incoming gate passes"),
      { cause: error }
    )
  }
}
...
export function useIncomingGatePassesByFarmerLink(
  ...
  const enabled =
    (options?.enabled ?? true) && farmerStorageLinkId.trim().length > 0
