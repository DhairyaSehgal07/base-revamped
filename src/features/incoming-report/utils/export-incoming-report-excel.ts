...
function getExportRowHeight(exportCells: ExportCellValue[], isGroupRow: boolean): number {
...
export async function exportIncomingReportToExcel({
  ...
  for (const row of exportRows) {
    ...
    const exportCells = visibleColumns.map((column) =>
      getExportCellForRow(row, column, quantityMode, undefined, showLocation),
    );
  ...
  const filename = sanitizeFilename(`incoming-report_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`);
