...
export function ChamberMap() {
  ...
  const [selectedFloor, setSelectedFloor] = useState<DemoFloor>(chamber.floors[1] ?? chamber.floors[0]);
  ...
  const animateInspector = () => {
    ...
    gsap.fromTo(inspectorRef.current, { y: 6, opacity: 0.4 }, { y: 0, opacity: 1, duration: 0.3, ease: 'power2.out' });
  };
  ...
  return (
    <section id="chamber-map" className="bg-muted/30 scroll-mt-24 border-y py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div data-reveal className="bg-card mt-14 grid grid-cols-1 gap-8 rounded-2xl border p-6 shadow-sm sm:p-8 lg:grid-cols-12">
          <div className="space-y-4 lg:col-span-7">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-3">
              ...
              <span className="flex gap-4 text-xs font-medium">
                ...
                <span className="flex items-center gap-1.5">
                  <span className={`size-2.5 rounded-full ${utilizationBandDotClass('medium')}`} /> Medium
                </span>
                <span className="flex items-center gap-1.5">
                  <span className={`size-2.5 rounded-full ${utilizationBandDotClass('high')}`} /> High
                </span>
              </span>
            </div>
              ...
              <div className="space-y-5 pt-5 text-sm">
                <div>
                  <div className="flex items-baseline justify-between gap-3">
                    ...
                    <p className="font-heading text-base font-bold tabular-nums">{formatUtilization(selectedPercent)}</p>
                  </div>
                  ...
                  <p className="text-muted-foreground mt-1.5 text-xs tabular-nums">
                    {formatBags(selectedFloor.bags)} / {formatBags(selectedFloor.capacity)} bags utilized
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                  <div>
                    ...
                    <p className="mt-1 font-medium tabular-nums">{formatBags(selectedFloor.bags)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground font-mono text-[10px] font-semibold tracking-wider uppercase">
                      Floor capacity
                    </p>
                    <p className="mt-1 font-medium tabular-nums">{formatBags(selectedFloor.capacity)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground font-mono text-[10px] font-semibold tracking-wider uppercase">
                      Available space
                    </p>
                    <p className="text-primary mt-1 font-semibold tabular-nums">{formatBags(selectedAvailable)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground font-mono text-[10px] font-semibold tracking-wider uppercase">
                      Share of chamber
                    </p>
                    <p className="mt-1 font-medium tabular-nums">{formatUtilization(selectedShare)}</p>
                  </div>
                </div>
