          ...
          {STEPS.map((step) => (
            <div key={step.num} className="card-hover bg-card rounded-xl border p-6">
              <div className="flex items-start justify-between">
                ...
                <span className="font-heading text-primary/15 text-5xl leading-none font-bold">{step.num}</span>
              </div>
              <h3 className="mt-5 text-lg font-semibold">{step.title}</h3>
              <p className="text-muted-foreground mt-2 text-sm leading-relaxed text-pretty">{step.desc}</p>
            </div>
          ))}
        </div>
      </div>
