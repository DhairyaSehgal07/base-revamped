...
export function useFarmerFinanceLedgers(linkId: string) {
  ...
  const potatoPurchaseLedger = useMemo(
    () =>
      allLedgers.find((ledger) => ledger.name === POTATO_PURCHASE_LEDGER_NAME) ??
      null,
    [allLedgers],
  )
  ...
  const otherIncomeLedger = useMemo(
    () =>
      allLedgers.find((ledger) => ledger.name === OTHER_INCOME_LEDGER_NAME) ??
      null,
    [allLedgers],
  )

  const paymentLedgers = useMemo(
    () => filterPaymentLedgers(allLedgers),
    [allLedgers],
  )
