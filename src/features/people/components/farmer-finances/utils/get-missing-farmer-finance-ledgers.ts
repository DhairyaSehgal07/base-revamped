...
function collectMissingLedgers(
  required: { label: string; ledger: Ledger | null }[],
): string[] {
  return required.filter((item) => !item.ledger).map((item) => item.label)
}

export function getMissingBuyPotatoLedgers({
  debitLedger,
  creditLedger,
}: LedgerPair): string[] {
...
export function getMissingSellPotatoLedgers({
  debitLedger,
  creditLedger,
}: LedgerPair): string[] {
  return collectMissingLedgers([
...
export function getMissingAddDiscountLedgers({
  debitLedger,
  creditLedger,
}: LedgerPair): string[] {
  return collectMissingLedgers([
...
export function getMissingAddChargeLedgers({
  debitLedger,
  creditLedger,
}: LedgerPair): string[] {
  return collectMissingLedgers([
...
export function getMissingFarmerPaymentLedgers({
  ...
  const missing = collectMissingLedgers([
    { label: "Farmer ledger", ledger: farmerLedger },
  ])
