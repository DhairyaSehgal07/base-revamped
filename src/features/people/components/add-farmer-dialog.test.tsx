...
function renderDialog(
  props: Partial<React.ComponentProps<typeof AddFarmerDialog>> = {},
) {
...
async function fillValidFarmerForm() {
  await user.type(
    screen.getByPlaceholderText(/enter 10-digit mobile number/i),
    '9812345678',
  );
  await user.type(
    screen.getByPlaceholderText(/enter farmer name/i),
    'New Farmer',
  );
  await user.type(
...
describe('AddFarmerDialog', () => {
  ...
  it('fills the suggested account number when Use suggested is clicked', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: /use suggested/i }),
    );
  ...
  it('shows duplicate account number validation', async () => {
    ...
    expect(
      await screen.findByText(/this account number is already in use/i),
    ).toBeInTheDocument();
  });
