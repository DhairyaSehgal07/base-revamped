...
function addCategoryTotal(
  ...
  const existing = categories.find(
    (item) =>
      item.subType === ledger.subType && item.category === ledger.category
  )
  if (existing) {
...
export function buildBalanceSheetReport(
  ...
  for (const [subType, total] of Object.entries(liabilitySubTypes)) {
    ...
    for (const category of liabilityCategories.filter(
  ...
  const sortedAssetSubTypes = Object.entries(assetSubTypes).sort(
    ([subTypeA], [subTypeB]) => {
      const indexA = ASSET_SUBTYPE_ORDER.indexOf(
        subTypeA as (typeof ASSET_SUBTYPE_ORDER)[number]
      )
      const indexB = ASSET_SUBTYPE_ORDER.indexOf(
        subTypeB as (typeof ASSET_SUBTYPE_ORDER)[number]
      )
  ...
  for (const [subType, total] of sortedAssetSubTypes) {
    ...
    for (const category of assetCategories.filter(
      (item) => item.subType === subType
    )) {
  ...
  const isBalanced =
    Math.abs(totalAssets - totalLiabilitiesAndEquity) < epsilon
