...
export const editLedgerFormSchema = z
  .object(ledgerFormFields)
  .superRefine((values, ctx) => {
    validateLedgerFormValues(values, ctx)
  })
...
export function parseClosingBalance(
  value: string
...
export function ledgerToFormValues(ledger: Ledger): EditLedgerFormValues {
  return {
    ...
    closingBalance:
      ledger.closingBalance === null ? "" : String(ledger.closingBalance),
  }
}
