      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-border pb-4">
          ...
          <p className="text-sm text-muted-foreground">
            Closing balances of all ledgers that form the basis of financial
            statements.
          </p>
        </CardHeader>
        <CardContent className="flex flex-col gap-6 p-4 sm:p-6">
          {TYPE_ORDER.filter((type) => report.groupedByType[type]?.length).map(
            (type) => {
              const groups = report.groupedByType[type]!
              const typeTotal = groups.reduce((sum, group) => sum + group.total, 0)
              const displayTotal =
                type === "Equity"
                  ? typeTotal + report.netProfitLoss
                  : typeTotal
              ...
              return (
                <TypeSectionTable
                  ...
                  netProfitLoss={
                    type === "Equity" ? report.netProfitLoss : undefined
                  }
                />
              )
            }
