...
import type {
  VarietyBreakdownResponse,
  VarietyBreakdownStockFilter,
} from "../types"

export const VARIETY_BREAKDOWN_QUERY_KEY = [
  "analytics",
  "variety-breakdown",
] as const
...
export async function fetchVarietyBreakdownApi(
  ...
  const { data } = await apiClient.get<VarietyBreakdownResponse>(
...
async function fetchVarietyBreakdown(
  ...
  try {
    ...
    throw new Error(
      getApiErrorMessage(error, "Failed to load variety breakdown"),
      { cause: error },
    )
  }
}
