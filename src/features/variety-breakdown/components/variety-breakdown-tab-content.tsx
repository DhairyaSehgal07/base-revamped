...
import {
  getStockSummaryAccent,
} from "@/features/people/components/farmer-stock-summary-table-styles"
import type { StockFilterTab, StockQuantityMode } from "@/features/people/utils/build-farmer-stock-summary"
...
const varietyBreakdownRouteApi = getRouteApi(
...
export function VarietyBreakdownTabContent({
  ...
  const orderedSizeNames = useMemo(
    () => tabs.map((item) => item.value),
    [tabs],
  )

  const totalQuantity = selectedSize
    ? getSizeQuantityForMode(selectedSize, quantityMode)
    : 0

  const farmerRows = useMemo(
    () =>
      buildFarmerTableRows(
        selectedSize?.farmerBreakdown ?? [],
        quantityMode,
      ),
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <div className="space-y-0 bg-muted/20 p-4">
