...
export function ViewFiltersSheet({ table }: ViewFiltersSheetProps) {
  ...
  const handleResetChanges = () => {
    const nextColumnState = getStoredOutgoingReportColumnState(
      table.getAllLeafColumns().map((column) => column.id),
    )
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger asChild>
        <Button
          ...
          aria-label="View outgoing report filters"
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 space-y-0.5 text-left">
              ...
              <SheetDescription className="text-xs leading-snug text-muted-foreground">
                Manage outgoing report filters, columns, grouping, and default
                view.
              </SheetDescription>
            </div>
          </div>
