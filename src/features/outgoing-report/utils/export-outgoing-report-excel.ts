...
function applyCellStyle(
  ...
  const numFmt =
    exportCell.kind === "number" ? getExcelNumFmt() : undefined
...
export type ExportOutgoingReportOptions = {
...
}: ExportOutgoingReportOptions): Promise<void> {
  ...
  for (const row of exportRows) {
    ...
    visibleColumns.forEach((column, columnIndex) => {
      ...
      applyCellStyle(cell, exportCell, {
        ...
        fontColorArgb:
          isGroupRow && exportCell.kind === "text"
            ? COLORS.primary
            : undefined,
      })
    })
