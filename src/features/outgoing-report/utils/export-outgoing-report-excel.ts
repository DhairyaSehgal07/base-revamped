...
export type OutgoingReportPreviewData = {
  title: string
  subtitle: string
...
export type OutgoingReportExcelPackage = {
  buffer: ArrayBuffer
  fileName: string
  preview: OutgoingReportPreviewData
}

export function buildOutgoingReportPreviewData({
  table,
  coldStorageName,
  quantityMode,
  showLocation = true,
...
}: BuildOutgoingReportExcelInput): OutgoingReportPreviewData {
  ...
  const totalsValues = visibleColumns.map((column, columnIndex) => {
    ...
    return exportCellValueToPrimitive(
      getFooterExportValue(column.id, filteredRows, quantityMode),
    )
  })

  const sectionRow = createSectionTitleRow("Outgoing Details", columnCount)
  ...
  return {
    ...
    fileName: sanitizeFilename(
      `outgoing-report_${format(generatedAt, "yyyy-MM-dd")}.xlsx`,
    ),
  }
}

export async function buildOutgoingReportExcelPackage(
  input: BuildOutgoingReportExcelInput,
): Promise<OutgoingReportExcelPackage> {
  const preview = buildOutgoingReportPreviewData(input)
  const generatedAt = input.generatedAt ?? new Date()
  const ExcelJS = await loadExcelJS()

  const columnIds = input.table
...
export async function exportOutgoingReportToExcel(
  input: BuildOutgoingReportExcelInput,
): Promise<void> {
  const { buffer, fileName } = await buildOutgoingReportExcelPackage(input)
