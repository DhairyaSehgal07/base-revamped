...
import type { OutgoingReportPreviewData } from "@/features/outgoing-report/utils/export-outgoing-report-excel"
...
export const OUTGOING_REPORT_DOWNLOAD_EXCEL_MESSAGE =
  "kf-outgoing-report-download-excel" as const

export const OUTGOING_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE =
  "kf-outgoing-report-download-excel-done" as const

export type PreviewOutgoingReportOptions = {
  preview: OutgoingReportPreviewData
...
export function buildOutgoingReportPreviewHtml({
  preview,
  coldStorageName,
  reportTitle = "Outgoing Report",
  generatedAt = new Date(),
}: PreviewOutgoingReportOptions): string {
  const metadataText = [
  ...
  <body>
    ...
    <div class="toolbar">
      <button
        ...
        class="primary"
        onclick="downloadOutgoingReportExcel()"
      >
        Download Excel
      </button>
    </div>
    <script>
      (function () {
        var DOWNLOAD_TYPE = ${JSON.stringify(OUTGOING_REPORT_DOWNLOAD_EXCEL_MESSAGE)};
        var DONE_TYPE = ${JSON.stringify(OUTGOING_REPORT_DOWNLOAD_EXCEL_DONE_MESSAGE)};
        ...
        window.downloadOutgoingReportExcel = function () {
          if (!window.opener) {
...
export function openOutgoingReportPreview(
  options: PreviewOutgoingReportOptions,
): Window {
  const html = buildOutgoingReportPreviewHtml(options)
