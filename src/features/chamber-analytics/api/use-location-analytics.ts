...
export const LOCATION_ANALYTICS_QUERY_KEY = [
  "analytics",
  "location-analytics",
] as const

export async function fetchLocationAnalyticsApi(): Promise<LocationAnalyticsResponse> {
  const { data } = await apiClient.get<LocationAnalyticsResponse>(
...
async function fetchLocationAnalytics(): Promise<LocationAnalyticsResponse> {
  try {
    ...
    throw new Error(
      getApiErrorMessage(error, "Failed to load location analytics"),
      { cause: error },
    )
  }
}
