...
const numericSortingFn: SortingFn<FilteredChamberOrder> = (
  rowA,
  rowB,
  columnId,
) => {
...
const dateSortingFn: SortingFn<FilteredChamberOrder> = (
  rowA,
  rowB,
  columnId,
) => {
...
export function ChamberOrdersTable({
  ...
  const totalBags = rows.reduce(
    (sum, row) => sum + row.original.totalBags,
    0,
  )
    ...
    <Card className="min-w-0">
      <CardHeader>
        <CardTitle className="font-heading text-base font-semibold text-foreground">
          You are viewing gate passes for chamber {chamberLabel} and floor{" "}
          {floorLabel}.
        </CardTitle>
      </CardHeader>
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <TableRow key={headerGroup.id} className="border-0">
                      {headerGroup.headers.map((header) => {
                        ...
                        const columnWidth =
                          COLUMN_WIDTHS[header.column.id] ?? "8rem"
                    ...
                    >
                      {row.getVisibleCells().map((cell) => {
                        const meta = cell.column.columnDef.meta as
                          ...
                          | undefined
                        const columnWidth =
                          COLUMN_WIDTHS[cell.column.id] ?? "8rem"
                    ...
                    {leafColumns.map((column, columnIndex) => {
                      ...
                      return (
                        <TableCell
                          ...
                          style={{ width: columnWidth }}
                          aria-label={
                            column.id === "bags" ? "column total" : undefined
                          }
                        >
                          {column.id === "bags" ? (
