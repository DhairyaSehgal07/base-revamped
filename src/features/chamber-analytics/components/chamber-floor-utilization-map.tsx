...
function UtilizationLegend() {
  return (
    <span className="flex gap-4 text-xs font-medium text-foreground">
      <span className="flex items-center gap-1.5">
        <span
          className={cn("size-2.5 rounded-full", utilizationBandDotClass("low"))}
        />
        Low
      </span>
      <span className="flex items-center gap-1.5">
        <span
          className={cn(
            ...
            utilizationBandDotClass("medium"),
          )}
        />
        Medium
      </span>
...
function UtilizationMiniCard({
  ...
  return (
    ...
    >
      <span
        className={cn(
          "text-xs font-semibold leading-none text-muted-foreground uppercase",
          italic && "italic",
        )}
      >
        {label}
      ...
      <div className="space-y-1.5">
        <span className="flex items-end justify-between gap-2">
          <span className="font-heading text-sm leading-none font-semibold tabular-nums text-foreground">
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            <h3 className="font-heading text-sm leading-none font-semibold text-foreground">
              {title}
            </h3>
        ...
        <span className="flex shrink-0 items-center gap-1 rounded-full border border-primary/20 bg-primary/10 px-2 py-0.5 text-xs font-semibold text-primary">
...
export function ChamberFloorUtilizationMap({
  ...
  const selectedChamber =
    chambers.find((item) => item.chamber === activeChamber) ?? chambers[0]

  const selectedChamberRow =
    chamberRows.find((row) => row.chamber === selectedChamber?.chamber) ??
    chamberRows[0] ??
    null
  ...
  const activeFloorName =
    floor && floor !== "all" && floorRows.some((row) => row.floor === floor)
      ? floor
      : null

  const selectedFloor =
    (activeFloorName
      ? floorRows.find((row) => row.floor === activeFloorName)
      : null) ??
    floorRows[0] ??
    null
  ...
  useEffect(() => {
    ...
    const hasValidSelection =
      Boolean(floor) && floor !== "all" && names.includes(floor)
  ...
  const chamberShareOfStore =
    totalStoreBags > 0
      ? (selectedChamberRow.quantity / totalStoreBags) * 100
      : 0
  ...
  const floorShareOfChamber =
    selectedFloor && chamberBags > 0
      ? (selectedFloor.quantity / chamberBags) * 100
      : 0

  return (
    <div className="flex flex-col gap-6 rounded-xl border border-border bg-card p-4 shadow-sm sm:p-6">
      ...
      <section className="space-y-3" aria-labelledby="chamber-utilization-heading">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Select a chamber to view its floors
          </p>
        </div>
      ...
      >
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Select a floor to view gate passes
          </p>
        </div>
