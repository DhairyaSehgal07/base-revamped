...
export function buildChamberSummaryCards(
  ...
  const totalStock = chambers.reduce(
    (sum, chamber) => sum + getChamberQuantity(chamber, tab),
    0,
  )

  const activeChambers = chambers.filter(
    (chamber) =>
      chamber.chamber !== NO_CHAMBER &&
      getChamberQuantity(chamber, tab) > 0,
  )
  ...
  const unassignedQty = unassigned
    ? getChamberQuantity(unassigned, tab)
    : 0
