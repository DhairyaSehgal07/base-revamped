...
function validateOutgoingQuantity(
  value: string,
  maxToIssue: number
): string | null {
...
function OutgoingAllocationSheetBody({
  ...
  const parsedIssuingNow =
    quantityInput.trim() === "" || Number.isNaN(issuingNow) ? 0 : issuingNow
    ...
    <>
      <div className="space-y-5 overflow-y-auto px-5 py-5">
        <Card size="sm" className="bg-muted/20 py-4 ring-border/50">
          <CardContent className="space-y-2 px-4">
            <div className="flex items-center gap-2 text-sm font-medium">
              <Package2 className="size-4 shrink-0 text-primary" />
              <span>
                Gate pass{" "}
        ...
        <Field data-invalid={quantityError ? true : undefined}>
          <FieldLabel htmlFor="outgoing-allocation-qty">
            Quantity to issue
          </FieldLabel>
          <Input
            {...numericInputProps}
