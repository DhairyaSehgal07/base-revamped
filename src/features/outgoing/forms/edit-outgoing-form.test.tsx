...
vi.mock(
  "@/features/transfer-stock/hooks/use-storage-gate-passes-for-farmer",
  () => ({
...
function setupLoadedEntry(
  entryOverrides: Parameters<typeof makeOutgoingDaybookEntry>[0] = {}
) {
  mockUseOutgoingDaybookEntry.mockReturnValue(
    makeOutgoingDaybookEntry(entryOverrides)
  )
...
describe("EditOutgoingForm", () => {
  ...
  it("shows not found when the entry is missing from cache", () => {
    ...
    renderWithProviders(
      <EditOutgoingForm gatePassId={OUTGOING_GATE_PASS_ID} />
    )
  ...
  it("shows a skeleton while storage passes are loading", () => {
    ...
    expect(
      screen.queryByTestId("transfer-gate-passes-section")
    ).not.toBeInTheDocument()
  })

  it("shows cannot edit when the entry is null", () => {
    mockUseOutgoingDaybookEntry.mockReturnValue(
      makeOutgoingDaybookEntry({ isNull: true })
    )
  ...
  it("PATCHes header-only changes after review and confirm", async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith(
      expect.stringMatching(/outgoing #24 updated/i),
