import { useMutation, useQueryClient } from "@tanstack/react-query"

import { DAYBOOK_QUERY_KEY } from "@/features/daybook/api/use-daybook"
import { DAYBOOK_SEARCH_QUERY_KEY } from "@/features/daybook/api/use-daybook-search"
import { INCOMING_GATE_PASSES_BY_FARMER_LINK_QUERY_KEY } from "@/features/incoming/api/use-incoming-gate-passes-by-farmer-link"
import type {
  CreateOutgoingGatePassPayload,
  CreateOutgoingGatePassResponse,
  OutgoingGatePassRecord,
} from "@/features/outgoing/types/api"
import apiClient, { getApiErrorMessage } from "@/lib/api-client"
import { gatePassNumberQueryKey } from "@/lib/gate-pass-number"

async function createOutgoingGatePassRequest(
  payload: CreateOutgoingGatePassPayload
): Promise<OutgoingGatePassRecord> {
  try {
    const { data } = await apiClient.post<CreateOutgoingGatePassResponse>(
      "/outgoing-gate-pass/",
      payload
    )

    if (data.status !== "Success" || !data.data) {
      throw new Error(data.message ?? "Failed to create outgoing gate pass")
    }

    return data.data
  } catch (error) {
    throw new Error(getApiErrorMessage(error, "Failed to create outgoing gate pass"), {
      cause: error,
    })
  }
}

export function useCreateOutgoingGatePass() {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: createOutgoingGatePassRequest,
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: DAYBOOK_QUERY_KEY })
      void queryClient.invalidateQueries({ queryKey: DAYBOOK_SEARCH_QUERY_KEY })
      void queryClient.invalidateQueries({
        queryKey: INCOMING_GATE_PASSES_BY_FARMER_LINK_QUERY_KEY,
      })
      void queryClient.invalidateQueries({
        queryKey: gatePassNumberQueryKey("outgoing"),
      })
    },
  })
}
