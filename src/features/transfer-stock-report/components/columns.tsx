...
const reportNumericSortingFn: SortingFn<TransferStockReportRecord> = (
  rowA,
  rowB,
  columnId,
) => {
...
const reportDateSortingFn: SortingFn<TransferStockReportRecord> = (
  rowA,
  rowB,
  columnId,
) => {
...
const reportEmptyAggregation: AggregationFn<TransferStockReportRecord> =
  () => null
const reportSumAggregation: AggregationFn<TransferStockReportRecord> = (
  columnId,
  leafRows,
) =>
...
function buildTransferStockReportColumns(
  sizes: string[],
): ColumnDef<TransferStockReportRecord>[] {
  const sizeColumns: ColumnDef<TransferStockReportRecord>[] = sizes.map(
    (bagSizeName) => ({
      id: `size-${bagSizeName}`,
      ...
      cell: ({ cell, getValue, row }) => {
        ...
        const items = row.original.items.filter(
          (item) => item.bagSize === bagSizeName,
        )
...
export const columns: ColumnDef<TransferStockReportRecord>[] =
  getTransferStockReportColumns([])
