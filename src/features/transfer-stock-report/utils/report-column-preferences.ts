...
function toColumnState(
  ...
  if (!preferences) {
    return {
      columnVisibility: getDefaultHiddenVisibility(columnIds),
      columnOrder: [],
    }
  }
  ...
  const columnVisibility = {
    ...
    ...preferences.hiddenColumnIds.reduce<VisibilityState>(
      (visibility, columnId) => {
        if (columnIdSet.has(columnId)) visibility[columnId] = false
        return visibility
      },
  ...
  return {
    columnVisibility,
    columnOrder: preferences.columnOrder.filter((columnId) =>
...
export function saveTransferStockReportColumnState(
  ...
  const preferences: StoredColumnPreferences = {
    version: 1,
    hiddenColumnIds: columnIds.filter(
