...
export function orderDetailLineKey(
  line: OutgoingGatePassAuditOrderDetail
): string {
...
export function buildAlignedOrderDetailRows(
  ...
  const beforeByKey = new Map(
    beforeLines.map((line) => [orderDetailLineKey(line), line])
  )
  const afterByKey = new Map(
    afterLines.map((line) => [orderDetailLineKey(line), line])
  )
