...
export const OUTGOING_GATE_PASS_EDITS_QUERY_KEY = [
  "outgoing-gate-pass-edits",
] as const
...
function normalizeOutgoingEditsPagination(
  ...
  const totalPages =
    raw.totalPages ?? (total === 0 ? 0 : Math.max(1, Math.ceil(total / limit)))
...
function outgoingGatePassEditsQueryKey(filters: OutgoingEditHistorySearch) {
  return [
    ...OUTGOING_GATE_PASS_EDITS_QUERY_KEY,
    toOutgoingEditHistoryParams(filters),
  ] as const
}
...
async function fetchOutgoingGatePassEdits(
  ...
  try {
    ...
    throw new Error(
      getApiErrorMessage(error, "Failed to load outgoing edit history"),
      { cause: error }
    )
  }
}
