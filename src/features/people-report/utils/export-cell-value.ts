...
export function getFilteredLeafRowCount(
  ...
  return table
    .getFilteredRowModel()
    .flatRows.filter((row) => row.original.kind === "gate-pass").length
}

export function getFilteredGatePassEntriesFromTable(
  table: Table<FarmerReportTableRow>,
) {
  return table
    .getFilteredRowModel()
    .flatRows.filter(
      (row) => row.original.kind === "gate-pass" && row.original.entry,
    )
...
export function getFooterExportValue(
  ...
  if (columnId === "rowBags") {
    const total = rows.reduce(
      (sum, row) => sum + (row.original.rowBags ?? 0),
      0,
    )
