...
export async function fetchDaybookSearchApi(
  ...
  try {
    const { data } = await apiClient.post<SearchApiResponse>(
      "/store-admin/search",
      {
    ...
    if (isAxiosError(error) && error.response?.status === 404) {
      return {
        incoming: [],
        outgoing: [],
        emptyMessage: getApiErrorMessage(
          error,
          "No orders found with this receipt number"
        ),
      }
    }
...
export function useDaybookSearch(
  params: DaybookSearchParams,
