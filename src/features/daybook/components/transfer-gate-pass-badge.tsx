...
export function TransferGatePassBadge({
  bagCount,
  typeLabel,
  tone,
}: TransferGatePassBadgeProps) {
  const styles = toneStyles[tone]

  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-muted/60 px-2.5 py-0.5 text-xs">
      <span className="font-medium tabular-nums text-foreground">
        {formatQuantity(bagCount)} bags
      </span>
      <span
        className={cn("size-1 shrink-0 rounded-full", styles.dot)}
        aria-hidden
      />
