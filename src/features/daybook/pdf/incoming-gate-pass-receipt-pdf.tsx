...
import type { IncomingGatePassReceiptPdfData } from "@/features/daybook/utils/build-incoming-gate-pass-pdf-data"
...
export type IncomingGatePassReceiptPdfProps = IncomingGatePassReceiptPdfData & {
    ...
    <View>
      <View style={styles.letterhead}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.reportTitleHeader}>Incoming Gate Pass</Text>
        </View>
        <View style={styles.headerRight}>
          <View style={styles.docRefBlock}>
            ...
            {manualParchi ? (
              <Text style={styles.docRefSub}>Manual: {manualParchi}</Text>
            ) : null}
          </View>
        </View>
      </View>
...
function IncomingGatePassReceiptPage({
  coldStorageName,
  coldStorageAddress,
  coldStorageLogo,
  gatePassLabel,
  ...
  bagRows,
  remarks,
}: IncomingGatePassReceiptPdfProps) {
      ...
      <View style={styles.body}>
        ...
        <View style={styles.panel}>
          <View style={styles.tableHeaderRow}>
            <Text style={[styles.col1, styles.colTextHeader]}>Bag Size</Text>
...
export default function IncomingGatePassReceiptPdf(
  props: IncomingGatePassReceiptPdfProps,
) {
  return (
    <Document
      author="Coldop"
      keywords="cold storage, incoming gate pass, receipt"
      subject="Incoming Gate Pass Receipt"
      title={`${props.coldStorageName} - ${props.gatePassLabel}`}
    >
      <IncomingGatePassReceiptPage {...props} />
    </Document>
  )
}
